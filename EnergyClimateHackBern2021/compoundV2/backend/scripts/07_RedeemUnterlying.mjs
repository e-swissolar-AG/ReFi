...
// Declare a const Variable to pass to the redeem Function of the FakecDai contract
const REDEEM_UNT_AMOUNT = web3.utils.toHex("1000000000000000000000");
console.log("5. The const variable has been declared");

// Create the encoded ABI of the redeemUnderlying function
const redeem_unt_EncodedABI = FakecDaiContractInstance.methods
  .redeemUnderlying(REDEEM_UNT_AMOUNT)
  .encodeABI();
...
function sendSignedTx(transactionObject, cb) {
    ...
    const privateKey = new Buffer.from(privKeyA, "hex");
...
web3.eth.getTransactionCount(ADDRESS_ALICE).then(transactionNonce => {
    ...
    console.log("9. The Transaction has been executed")
},
